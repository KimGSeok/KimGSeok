...
try {
  ...
  writeFileSync(
    ...
    '"use client"; import { Button } from "@kimgseok/design-button/web"; import { colors } from "@kimgseok/design-tokens/colors"; import { Text } from "@kimgseok/design-primitives/web"; import { ToastViewport } from "@kimgseok/design-feedback/web"; import { Tabs } from "@kimgseok/design-navigation/web"; import { Calendar, DatePicker, DateRangePicker } from "@kimgseok/design-date-picker/web"; const noop=()=>{}; export default function Client(){ return <><Text color={colors.red500} textStyle="text-l-medium">토큰 검증</Text><Button onAction={noop}>확인</Button><ToastViewport toast={null} /><Tabs accessibilityLabel="검증" items={[{value:"a",label:"A",content:"A"}]} onValueChange={noop} value="a" /><Calendar accessibilityLabel="Calendar" locale="en-US" onValueChange={noop} value="2026-08-24" /><DatePicker label="Date" locale="en-US" onValueChange={noop} value="2026-08-24" /><DateRangePicker label="Range" locale="en-US" onValueChange={noop} value={{start:"2026-08-24",end:"2026-08-25"}} /></> }',
  );
  writeFileSync(
