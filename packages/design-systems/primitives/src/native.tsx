...
function NativeSkeleton({ shape = 'text', size = 'md', mode }: SharedSkeletonProps & { mode?: 'light' | 'dark' }) { assertSkeletonContract({ shape, size }); const theme = useTheme(mode); const reduced = useReduceMotion(); const opacity = useRef(new Animated.Value(1)).current; useEffect(() => { if (reduced !== false) { opacity.setValue(1); return; } const pulse = Animated.loop(Animated.sequence([Animated.timing(opacity, { duration: 320, toValue: 0.58, useNativeDriver: true }), Animated.timing(opacity, { duration: 320, toValue: 1, useNativeDriver: true })])); pulse.start(); return () => pulse.stop(); }, [opacity, reduced]); const width = size === 'sm' ? 72 : size === 'md' ? 144 : size === 'lg' ? 240 : '100%'; const height = shape === 'text' ? size === 'sm' ? 12 : size === 'md' ? 16 : 20 : size === 'sm' ? 32 : size === 'md' ? 48 : 72; const circle = shape === 'circle'; return <Animated.View accessible={false} style={{ backgroundColor: theme.color.semantic.bg.raised, borderRadius: circle ? height / 2 : theme.foundation.radius.md, height, maxWidth: '100%', opacity, width: circle ? height : width }} />; }
export function NativeSkeletonRegion({ accessibilityLabel, items, mode }: SharedSkeletonRegionProps & { mode?: 'light' | 'dark' }) { assertSkeletonRegionContract({ accessibilityLabel, items }); return <View accessible accessibilityLabel={accessibilityLabel} accessibilityRole="summary" accessibilityState={{ busy: true }} style={{ gap: 12, width: '100%' }}>{items.map((item, index) => <NativeSkeleton {...item} key={`${item.shape ?? 'text'}-${item.size ?? 'md'}-${index}`} mode={mode} />)}</View>; }
