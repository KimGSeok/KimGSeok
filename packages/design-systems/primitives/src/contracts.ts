...
export type SkeletonRecipe = 'text-block' | 'list-item' | 'avatar-row' | 'card' | 'table-row' | 'form';
export interface SharedSkeletonRegionProps { accessibilityLabel: string; items?: readonly SharedSkeletonProps[]; recipe?: SkeletonRecipe; count?: number; }
export function assertSkeletonRegionContract({ accessibilityLabel, items, recipe, count }: SharedSkeletonRegionProps) { const hasItems = Array.isArray(items); const hasRecipe = recipe !== undefined; if (!accessibilityLabel.trim() || hasItems === hasRecipe || hasItems && items.length === 0 || hasRecipe && !['text-block', 'list-item', 'avatar-row', 'card', 'table-row', 'form'].includes(recipe) || count !== undefined && (!Number.isInteger(count) || count < 1 || count > 8) || hasItems && count !== undefined) throw new Error('SkeletonRegion contract is invalid.'); items?.forEach(assertSkeletonContract); }
export function getSkeletonRecipeCount(recipe: SkeletonRecipe, count?: number) { return count ?? (recipe === 'list-item' || recipe === 'table-row' ? 3 : 1); }
